...
func runApp(cmd *Command, args []string) int {
	...
	if len(args) == 0 || args[0] == "watchall" {
		...
		helper.ColorLog("[INFO] Uses '%s' as 'appname'\n", appname)
	} else {
		appname = args[0]
	}
	helper.Debugf("current path:%s\n", crupath)

	err := loadConfig()
	if err != nil {
		helper.ColorLog("[ERRO] Fail to parse bee.json[ %s ]\n", err)
	}
	...
	gps := helper.GetGOPATHs()
	if len(gps) == 0 {
		helper.ColorLog("[ERRO] Fail to start[ %s ]\n", "$GOPATH is not set or empty")
		os.Exit(2)
	}
	gopath := gps[0]
